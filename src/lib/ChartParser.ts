...
const getSustainLength = (pos: number, duration: number, timing: TP[], resolution: number) => {
	let tp: TP = { point: 0, ms: 0, measureLength: 0 };
	for (const point of timing) {
		if (point.point <= pos) {
			tp = point;
...
export const getTiming = (data: string[][], resolution: number): [ChartFile.Timing[], TP[]] => {
...
const getEvents = (data: string[][], timing: TP[], resolution: number): ChartFile.Event[] => {
...
const getNotes = (data: string[][], timing: TP[], resolution: number): ChartFile.Note[] => {
	...
	for (const timingPoint in data) {
		...
		for (const note of notes) {
			if (note.match(/N \d+ \d+/)) {
				...
				if (parseInt(type) == 7) {
					point.notes[5] = true;
					durationNum > 0
						? (point.duration[5] = getSustainLength(point.point, durationNum, timing, resolution))
						: '';
					continue;
				}
		...
		// Is Powered?
		point.powered = point.point >= starPower.startPosition && point.point < starPower.endPosition;
		previousNote = point;
		out.push(point);
	}
